...
DEMO_SNAPSHOT = os.path.join(BASE_DIR, "demo_snapshot.json")
...
def login():
    ...
    return jsonify({"ok": True, "user": {
        ...
        "vis_he_can_see": _col("vis_he_can_see", "[]"),
        "vis_can_see_me": _col("vis_can_see_me", "[]"),
...
def _visible_scope(username, block_id=None):
    ...
    conn.close()
    ph = ",".join("?" * len(visible))
    # 两个分支都必须 lower：owner_user_id 实际存的是用户名字符串（历史列名），
    # 存量数据大小写混杂（如 'Khoa'），不 lower 会导致“自己的行不可见”
    clause = (f"(owner_user_id <> '' AND lower(owner_user_id) IN ({ph})) "
              f"OR (owner_user_id = '' AND lower(salesperson) IN ({ph}))")
...
def create_project():
    ...
    for f in fields:
        if f in data:
            cols.append(f)
            v = data[f]
            if f.startswith(("equip_","superv_","fb_","cc_","inst_","total_","cust_","gs_","agi_","vat_","doc_")) and f not in ("cust_remark","gs_remark","gs_comm_pct","agi_remark","doc_remark","doc_equip","doc_install","doc_both"):
                try:
                    v = int(v or 0)
                except:
                    v = 0
...
def delete_project(pid):
    ...
    for a in atts:
        try:
            os.remove(os.path.join(UPLOAD_DIR, a.get("filename")))
        except:
            pass
...
def list_crm_projects():
    ...
    sql = ("SELECT * FROM crm_projects WHERE (%s)" % base)
    if clause:
        sql += " AND (%s)" % clause
    sql += " ORDER BY id ASC"
    rows = conn.execute(sql, params).fetchall()
...
def delete_crm_project(pid):
    ...
    _rm_upload_files(row["pdf_equip"], row["pdf_install"], row["pdf_both"])
    conn.commit()
    conn.close()
    return jsonify({"ok":True})
...
def restore_lost_project(pid):
    ...
    vals = [d.get(c, "") if c not in CRM_NUM_FIELDS else int(d.get(c) or 0) for c in cols]
    conn.execute(
        ...
        try:
            if new_status == "approved":
                if row["block_id"] == "USR-D1":
                    ...
                    _uname = (pl.get("username") or pl.get("login") or "").strip()   # 前端 payload 用 login 字段
                    if _uname:
...
OBSERVER_FIXED_PERMS = {
    ...
    "CRM-L1": "company", "CRM-D1": "default", "CRM-D2": "hide", "CRM-D3": "hide",
    ...
    "WON-P345": "hide",
    "LOST-L1": "company", "LOST-D1": "default", "LOST-D2": "hide", "LOST-D3": "hide",
...
def api_users_create():
    ...
    cur = conn.execute(
        ...
        (username, DEFAULT_INIT_PASSWORD, real_name, "user", position, status,
         ...
         _json.dumps(perms), _json.dumps(fa[:3], ensure_ascii=False), created_at)
    )
    conn.commit()
    new_id = cur.lastrowid
    conn.close()
...
def _livestock_log_update(conn, item, country, ym, status, detail=""):
    conn.execute("""CREATE TABLE IF NOT EXISTS livestock_update_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        update_time TEXT NOT NULL,
        item TEXT, country TEXT, year_month TEXT,
        status TEXT, detail TEXT)""")
    ts = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute("""INSERT INTO livestock_update_log
      (update_time,item,country,year_month,status,detail) VALUES(?,?,?,?,?,?)""",
      (ts, item, country, ym, status, detail[:500]))
    conn.commit()
...
def _livestock_fetch_yahoo(symbol, period1, period2):
    """复用金属模块的雅虎抓取（瘦肉猪/鸡蛋期货日线）"""
...
def _month_has_data(cache, keys, cur_ym):
    """检查缓存中是否已有当前月(YYYY-MM)的数据。只要有任意一个 key 有当月数据，即视为已采集。"""
    for k in keys:
        m = cache.get(k, {})
        for d in m.keys():
            if d.startswith(cur_ym):
                return True
    return False
...
def clear_demo_data(c, also_business=True):
    ...
    注意：本函数会清空整张业务表。由于 DEMO 数据与非 DEMO 数据混在同一张表，
    无法区分，所以"清空"即"清表"。学员在演示期间录入的业务数据也会一并清掉 ——
    这正是培训场景需要的（每次演示从干净状态开始）。
    """
    removed = {}
    if also_business:
